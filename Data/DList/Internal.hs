...
--
-- This module exists only to export the 'DList' constructor and selector.
--
-- Note that unexpected things can happen if you don't construct 'DList' values
-- carefully.
--
-- @DList a@ is isomorphic to @[a]@ via 'fromList'/'toList', but @[a] -> [a]@ is
-- not isomorphic to @[a]@. That is, all lists are dlists, and all dlists are
-- lists /only/ if there is no way to construct an arbitrary @y :: DList a@
-- from an "unsafe" @x :: [a] -> [a]@.
--
-- This module exports the constructor 'DL' and deconstructor 'unDL', which
-- allow the construction of unsafe dlist values. Consider this GHCi session:
--
-- >>> let x = DL $ \z -> z ++ z
-- >>> toList x
-- []
...
--
-- >>> append x (singleton 'a')
-- Data.DList.fromList "aa"
-- >>> append (fromList $ toList x) (singleton 'a')
