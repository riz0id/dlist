...
prop_List_iso :: [Int] -> Bool
prop_List_iso = eqWith id (toList . fromList)
...
prop_snoc xs c = xs ++ [c] == toList (fromList xs `snoc` c)
...
prop_map_map :: (Int -> Int) -> (a -> Int) -> [a] -> Bool
prop_map_map f g =
...
props =
  [ ("List isomorphism",     property prop_List_iso)
